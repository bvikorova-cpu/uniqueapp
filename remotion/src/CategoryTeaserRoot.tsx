...
export const CategoryTeaserRoot: React.FC = () => (
  <>
    {(Object.keys(CATEGORY_TEASERS) as CategoryTeaserId[]).map((id) => (
      <Composition key={id} id={`category-${id}`} component={CategoryTeaser} durationInFrames={getCategoryTeaserDuration(id)} fps={30} width={1080} height={1920} defaultProps={{ id }} />
    ))}
  </>
);
