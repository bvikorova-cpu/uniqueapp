...
BEGIN
  ...
  WITH earned AS (
    ...
    WHERE created_at >= wk_start AND amount > 0
      AND COALESCE(source,'') NOT IN ('xp_sync','backfill','migration','badge_backfill')
    GROUP BY user_id
    UNION ALL
    SELECT user_id, SUM(points_earned)::int AS xp
    FROM public.activity_logs
    WHERE created_at >= wk_start AND points_earned > 0 AND points_earned <= 500
      AND COALESCE(activity_type,'') NOT IN ('xp_sync','backfill','migration','badge_earned')
      AND user_id NOT IN (
