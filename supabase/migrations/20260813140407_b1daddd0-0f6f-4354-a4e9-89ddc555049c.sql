-- Entry fee lowered to 100 coins (1 AI credit)
...
BEGIN
  ...
  v_balance_after := public.battle_coins_apply(v_user_id, -100, 'battle_entry', 'kitchenstars', _battle_id);
...
BEGIN
  ...
  v_balance_after := public.battle_coins_apply(v_user_id, -100, 'battle_entry', 'reel_battles', _battle_id);
...
-- Settlement: pot based on 100-coin entries, winner 80%, pool 20%
...
UPDATE public.kitchen_battles SET prize_pool = LEAST(prize_pool, 200) WHERE status = 'open';
UPDATE public.reel_battles SET prize_pool = LEAST(prize_pool, 200) WHERE status = 'open';