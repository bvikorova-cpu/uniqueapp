...
BEGIN
  ...
  SELECT
    ...
    p.avatar_url,
    s.country_code,
    COALESCE(s.best_iq, 0)::INTEGER,
    COALESCE(s.avg_iq, 0)::NUMERIC,
    COALESCE(s.total_tests, 0)::INTEGER,
    COALESCE(s.tier, 'Bronze')::TEXT,
    (p.user_id = _uid) AS is_me
...
GRANT EXECUTE ON FUNCTION public.get_iq_friend_comparison(UUID) TO authenticated;