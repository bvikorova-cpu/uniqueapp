-- Secure server-side mystery-box Lucky Wheel: prize picked by server using weighted random.
-- Client no longer chooses the prize (fixes critical reward-fraud vector).
...
DECLARE
  ...
  v_prize_label text := '';
  -- Weighted prize table (must match client UI labels & values).
  -- (value, label, chance_pct)
...
BEGIN
  ...
  -- Server-side weighted pick using pgcrypto-quality random
  v_rand := random() * 100;
...
-- Disable insecure client-trusted variant (callers redirected to *_secure).
REVOKE EXECUTE ON FUNCTION public.lucky_wheel_spin(integer, integer, text) FROM authenticated, PUBLIC, anon;