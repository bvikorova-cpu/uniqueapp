...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    PERFORM public.award_xp(v_user, v_xp, 'daily_login_streak', v_today::text);
    RETURN jsonb_build_object('claimed', true, 'streak', 1, 'xp', v_xp, 'bonus', v_xp, 'was_reset', false);
  END IF;
  ...
  -- XP reward (no credits): 50 XP base, +50 for every completed 7-day streak, capped at 500 XP
  v_xp := LEAST(500, 50 * (1 + (v_new_streak / 7)));
  ...
  RETURN jsonb_build_object('claimed', true, 'streak', v_new_streak, 'xp', v_xp, 'bonus', v_xp, 'was_reset', v_was_reset, 'missed_days', v_missed_days);
END;
$$;