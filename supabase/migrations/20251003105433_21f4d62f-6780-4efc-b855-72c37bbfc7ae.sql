...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'megatalent_tier') THEN
    CREATE TYPE public.megatalent_tier AS ENUM ('basic', 'top_premium');
  END IF;
END $$;

-- Create table for Megatalent subscriptions
CREATE TABLE IF NOT EXISTS public.megatalent_subscriptions (
...
DROP POLICY IF EXISTS "Users can view their own subscription" ON public.megatalent_subscriptions;
DROP POLICY IF EXISTS "Users can create their own subscription" ON public.megatalent_subscriptions;
DROP POLICY IF EXISTS "Users can update their own subscription" ON public.megatalent_subscriptions;
...
DROP POLICY IF EXISTS "Anyone can view votes" ON public.talent_votes;
DROP POLICY IF EXISTS "Authenticated users can vote" ON public.talent_votes;
DROP POLICY IF EXISTS "Users can delete their own votes" ON public.talent_votes;
...
DROP TRIGGER IF EXISTS update_votes_count_on_vote ON public.talent_votes;
DROP TRIGGER IF EXISTS add_bonus_votes_on_submission ON public.talent_submissions;
DROP TRIGGER IF EXISTS update_megatalent_subscriptions_updated_at ON public.megatalent_subscriptions;
DROP TRIGGER IF EXISTS update_talent_submissions_updated_at ON public.talent_submissions;
DROP FUNCTION IF EXISTS public.update_submission_votes_count();
DROP FUNCTION IF EXISTS public.add_premium_bonus_votes();

-- Create function to update votes count
CREATE FUNCTION public.update_submission_votes_count()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE FUNCTION public.add_premium_bonus_votes()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
