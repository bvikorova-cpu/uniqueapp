CREATE OR REPLACE FUNCTION public.coffee_discover_candidates(_limit integer DEFAULT 20)
 RETURNS TABLE(user_id uuid, full_name text, avatar_url text, bio text, favorite_coffee_types text[], preferred_atmosphere text[], budget_preference text, total_checkins integer)
 LANGUAGE sql
