...
BEGIN
  ...
  WITH earned AS (
    ...
    WHERE created_at >= wk_start AND amount > 0
      AND COALESCE(source,'') NOT IN ('xp_sync','backfill','migration')
    GROUP BY user_id
    UNION ALL
    SELECT user_id, SUM(points_earned)::int AS xp
    FROM public.activity_logs
    WHERE created_at >= wk_start AND points_earned > 0
      AND COALESCE(activity_type,'') NOT IN ('xp_sync','backfill','migration')
  ...
  WHERE u.season_id = s.id
    AND u.weekly_xp > 0
    AND u.user_id NOT IN (
      SELECT user_id FROM public.xp_events WHERE created_at >= wk_start AND amount > 0
        AND COALESCE(source,'') NOT IN ('xp_sync','backfill','migration')
      UNION
      SELECT user_id FROM public.activity_logs WHERE created_at >= wk_start AND points_earned > 0
        AND COALESCE(activity_type,'') NOT IN ('xp_sync','backfill','migration')
    );
