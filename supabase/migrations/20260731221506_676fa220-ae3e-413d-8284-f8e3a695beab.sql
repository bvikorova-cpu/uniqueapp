...
BEGIN
  ...
  LOOP
    ...
    SELECT id, title, creator_id, total_revenue_eur INTO v_item
    ...
    IF v_item.creator_id IS NOT NULL THEN
      ...
      VALUES (
        ...
        '"' || COALESCE(v_item.title,'Your asset') || '" sold for €' || to_char(v_total,'FM999999990.00')
          || ' — €' || to_char(v_creator,'FM999999990.00') || ' (70%) was added to your wallet.',
        '/stock-content-library',
        v_item.id,
        jsonb_build_object('content_id', v_item.id, 'amount_paid', v_total, 'creator_earning', v_creator)
      );
    END IF;
...
GRANT EXECUTE ON FUNCTION public.finalize_stock_content_sale(text) TO authenticated;