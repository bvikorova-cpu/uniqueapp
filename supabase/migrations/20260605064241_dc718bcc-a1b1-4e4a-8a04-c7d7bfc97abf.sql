...
-- Atomic daily-limit guard for Lucky Wheel
ALTER TABLE public.lucky_spin_log
  ADD COLUMN IF NOT EXISTS spin_date date
  GENERATED ALWAYS AS (((created_at AT TIME ZONE 'UTC'))::date) STORED;

CREATE UNIQUE INDEX IF NOT EXISTS lucky_spin_log_user_day_uniq
  ON public.lucky_spin_log (user_id, spin_date);
...
BEGIN
  ...
  -- Insert log FIRST — unique (user_id, spin_date) enforces atomic daily limit.
  BEGIN
