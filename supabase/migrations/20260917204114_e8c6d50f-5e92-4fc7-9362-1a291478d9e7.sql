...
AS $function$
  ...
  SELECT p.id, p.full_name, p.username, p.avatar_url, p.cover_url, p.headline,
         NULLIF(trim(COALESCE(p.city, p.location, '')), '') AS city,
         p.is_verified
  ...
  WHERE auth.uid() IS NOT NULL AND p.id <> auth.uid() AND length(params.qn) >= 1
    AND (public.f_unaccent(lower(coalesce(p.full_name,''))) ILIKE '%' || params.qn || '%'
      ...
      OR public.f_unaccent(lower(coalesce(p.location,''))) ILIKE '%' || params.qn || '%')
  ORDER BY CASE
    ...
    WHEN public.f_unaccent(lower(coalesce(p.city, p.location, ''))) ILIKE params.qn || '%' THEN 2 ELSE 3 END,
