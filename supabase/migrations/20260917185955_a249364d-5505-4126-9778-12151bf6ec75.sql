ALTER TABLE public.posts
  ADD COLUMN IF NOT EXISTS premium_video_id uuid REFERENCES public.premium_videos(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_posts_premium_video_id ON public.posts(premium_video_id) WHERE premium_video_id IS NOT NULL;
...
AS $function$
  ...
  public_posts AS (
    SELECT p.id, p.user_id, p.content, p.created_at, p.likes_count,
           ...
           p.feeling, p.location, p.privacy, p.audience,
           p.community_id, p.is_sensitive, p.event_id, p.premium_video_id
    FROM public.posts p
    WHERE (_cursor IS NULL OR p.created_at < _cursor)
      AND (p.privacy = 'public' OR p.privacy IS NULL)
      AND coalesce(p.audience, 'public') <> 'close_friends'
  ...
  own_posts AS (
    ...
    WHERE v.uid IS NOT NULL
      ...
      AND (
        (p.privacy IS NOT NULL AND p.privacy <> 'public')
        OR coalesce(p.audience, '') = 'close_friends'
      )
    ORDER BY p.created_at DESC
    LIMIT _limit
  ),
  ...
  friends_posts AS (
    ...
    WHERE v.uid IS NOT NULL
      ...
      AND (
        coalesce(p.audience, 'friends') <> 'close_friends'
        OR EXISTS (
          SELECT 1 FROM public.user_close_friends cf
          WHERE cf.user_id = p.user_id AND cf.friend_id = v.uid
        )
      )
    ORDER BY p.created_at DESC
    LIMIT _limit
  ...
  followers_posts AS (
    ...
    WHERE v.uid IS NOT NULL
      ...
      AND coalesce(p.audience, 'followers') <> 'close_friends'
    ORDER BY p.created_at DESC
    LIMIT _limit
  ),
  ...
  posts_final AS (
    SELECT id, user_id, content, created_at, likes_count, comments_count,
           ...
           audience, community_id, is_sensitive, event_id, premium_video_id, tier_rank
  ...
  ref_original_posts AS (
    SELECT op.id, op.user_id, op.content, op.created_at, op.likes_count,
           ...
           op.feeling, op.location, op.privacy, op.audience, op.event_id,
           op.premium_video_id
    FROM public.posts op
    WHERE op.id IN (SELECT original_post_id FROM reposts_page)
  )
