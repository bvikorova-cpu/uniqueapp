...
CREATE OR REPLACE FUNCTION public.gift_ai_credits_by_identifier(
  p_recipient TEXT,
  p_amount INT,
...
BEGIN
  ...
  -- 3) full name (exact, case-insensitive)
  IF v_recipient IS NULL THEN
    SELECT id, COALESCE(username, full_name, email) INTO v_recipient, v_recipient_label
...
REVOKE ALL ON FUNCTION public.gift_ai_credits_by_identifier(TEXT,INT,TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.gift_ai_credits_by_identifier(TEXT,INT,TEXT) TO authenticated;
