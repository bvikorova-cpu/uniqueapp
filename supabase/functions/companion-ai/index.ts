    ...
    switch (action) {
      ...
      case "memory-analyze":
        result = await callAI(apiKey, [
          {
            role: "system",
            content: `Analyze this conversation and extract key memory points about the user. Return JSON: {"summary":"2-3 sentence summary of relationship and topics","memory_context":{"name":"user's name if mentioned","interests":["list"]);
        break;
      case "mood-matcher":
        result = await callAI(apiKey, [
          {
