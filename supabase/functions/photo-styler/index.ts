...
const STYLE_PROMPTS: Record<string, string> = {
  ...
  uniquemascot: "a fun photo of every person from the user photo standing next to a full-body walking plush mascot character in a purple-to-hot-pink gradient costume: a big rounded head shaped like the Unique logo tile with a large white calligraphic script 'U' and a white four-point sparkle on the front of the head, plus a chubby plush body, two short arms with mitten hands and two short legs with big soft feet, standing upright on the ground at human height. The head has NO eyes, NO mouth, NO nose, NO facial features at all, and the mascot's body carries NO text or website address - only the U and the sparkle. The mascot waves one hand at the camera. Directly under the mascot render the official Unique app logo: a glossy rounded-square icon with a smooth gradient from violet-purple (left) to hot pink (right), a large white calligraphic script letter 'U' centred on it and a bright four-point white sparkle star at the top right of the U, and the exact clean sans-serif text 'www.uniqueapp.fun' - spelled exactly that way, sharp and perfectly legible. Cheerful bright scene, everyone facing camera",
