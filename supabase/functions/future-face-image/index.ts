...
const agingDetail = (yearsRaw: unknown) => {
  ...
  const tiers: Array<{ upTo: number; text: string }> = [
    ...
    { upTo: 20, text: "Clearly etched forehead and glabella lines, defined crow's feet and tear troughs, visible nasolabial folds and early marionette lines, noticeable loss of mid-face volume, mild upper-eyelid hooding, visible pores and uneven pigmentation, roughly 20-30% grey hair, slightly softened jawline." },
  ...
  return `The subject must look EXACTLY ${y} years older than in the reference photo — a viewer must be able to guess the added ${y} years without seeing the original. Required for +${y} years: ${tier} Do NOT keep the current-age skin, hair colour or facial firmness.`;
};

const ACTIONS: Record<string, { cost: number; prompt: (p: any) => string }> = {
  age_progression: { cost: 6, prompt: ({ years }) => `Age the person in the reference photo forward in time. This is a hard age transformation, not a photo enhancement or beauty retouch. ${agingDetail(years)} Rebuild age-dependent facial anatomy from scratch: soft-tissue proportions, wrinkle pattern, skin texture and laxity, fat distribution, hair colour and density. Keep the same person, pose, clothing and background. ${visibleEdit}` },
