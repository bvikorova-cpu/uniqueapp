...
serve(async (req) => {
  ...
  try {
    ...
    if (customers.data.length === 0) { await admin.from("iq_subscriptions").upsert({
        user_id: user.id, tier: "free", status: "inactive" }, { onConflict: "user_id" });
    ...
    if (!sub) { await admin.from("iq_subscriptions").upsert({
        user_id: user.id, tier: "free", status: "inactive",
        stripe_customer_id: customerId }, { onConflict: "user_id" });
      return new Response(JSON.stringify({ subscribed: false, tier: "free" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
