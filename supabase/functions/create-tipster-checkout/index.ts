...
const TIPSTER_PRICE_ID = "price_1STtBV0QTWhd4oRpJUGoW9Lk";

const logStep = (step: string, details?: any) => {
  const detailsStr = details ? ` - ${JSON.stringify(details)}` : '';
  console.log(`[CREATE-TIPSTER-CHECKOUT] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price: TIPSTER_PRICE_ID,
          quantity: 1,
        },
      ],
      mode: "subscription",
      success_url: `${req.headers.get("origin")}/sports-predictor?tipster_success=true`,
