...
serve(async (req) => {
  ...
  try {
    const { height, bodyShape, styleGoal } = await req.json();
    ...
    const result = toolCall ? JSON.parse(toolCall.function.arguments) : { shapeAnalysis: aiData.choices?.[0]?.message?.content };
