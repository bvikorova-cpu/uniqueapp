...
const SYSTEM = `Pick healthy menu items. Return JSON: {top_picks:[{item, why, est_calories}], avoid:[{item, why}], modifications[]}.`;
...
serve(async (req) => {
  ...
  try {
    ...
    const creditDenied = await deductAICredits(user.id, 2, "analyze-restaurant-menu");
