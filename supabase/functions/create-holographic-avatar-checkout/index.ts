...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[CREATE-HOLOGRAPHIC-AVATAR-CHECKOUT] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    const isSubscription = priceId === "price_1SPjFEGaXSfGtYFtBjeXRVkk"; // Premium AI Avatar
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${req.headers.get("origin")}/holographic-avatars?success=true`,
