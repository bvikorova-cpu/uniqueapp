...
serve(async (req) => {
  ...
  try {
    const { preferences, bodyType, lifestyle } = await req.json();
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        tool_choice: { type: "function", function: { name: "style_dna_result" } }
      })
    });
    ...
    const result = toolCall ? JSON.parse(toolCall.function.arguments) : { styleProfile: aiData.choices?.[0]?.message?.content || "Analysis complete" };
