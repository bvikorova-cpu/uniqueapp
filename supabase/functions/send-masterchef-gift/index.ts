...
const GiftRequestSchema = z.object({ chefId: z.string().uuid("Invalid chef ID"),
...
serve(async (req) => {
  ...
  try {
    ...
    const { error: insertError } = await supabase.from("masterchef_sent_gifts").insert({ sender_id: user.id,
      chef_id: chefId,
      gift_id: giftId,
