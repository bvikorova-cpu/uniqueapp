...
const VOTE_PACKAGES: Record<string, number> = { "price_1SSDabGaXSfGtYFtjBhb6kVr": 5,
...
serve(async (req) => {
  ...
  try {
    ...
    // Record verification
    await supabaseAdmin.from("payment_verifications").upsert({ stripe_session_id: sessionId,
      user_id: user.id,
