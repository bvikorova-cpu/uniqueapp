// Verify a crystal marketplace Stripe checkout session and activate the order.
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: updated, error } = orderId
      ? await query.eq("id", orderId).select().maybeSingle()
      : await query.eq("stripe_payment_id", sessionId).select().maybeSingle();
    ...
    console.error("verify-crystal-purchase error:", error);
    return new Response(
