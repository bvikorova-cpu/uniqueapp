...
const SYSTEM = `Hydration coach. Return JSON: {daily_ml, schedule:[{time, ml, reminder}], electrolyte_advice}.`;
...
serve(async (req) => {
  ...
  try {
    ...
    const creditDenied = await deductAICredits(user.id, 2, "nutrition-hydration-coach");
