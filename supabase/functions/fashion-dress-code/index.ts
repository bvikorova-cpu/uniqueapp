...
serve(async (req) => {
  ...
  try {
    ...
    if (!credits || credits.credits_remaining < 6) throw new Error("Insufficient credits (need 6)");
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: `What should I wear to: ${eventDescription}` }
        ], max_tokens: 2000
      })
    });
    const aiData = await response.json();
    await supabase.from("ai_credits").update({ credits_remaining: credits.credits_remaining - 6 }).eq("user_id", user.id);
