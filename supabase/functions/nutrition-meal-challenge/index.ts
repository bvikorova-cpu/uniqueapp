...
const SYSTEM = `Create nutrition challenge. Return JSON: {challenge_name, duration_days, daily_tasks[], rewards[], difficulty}.`;
...
serve(async (req) => {
  ...
  try {
    ...
    const creditDenied = await deductAICredits(user.id, 3, "nutrition-meal-challenge");
