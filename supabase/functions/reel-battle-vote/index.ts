...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: battle, error: be } = await supabase
      .from("reel_battles").select("id, status, deadline").eq("id", battleId).single();
    ...
    const { data: existing } = await supabase
      .from("reel_battle_votes")
      .select("id")
      .eq("battle_id", battleId).eq("voter_id", user.id).maybeSingle();
    if (existing) throw new Error("You already used your single vote in this duel");

    const { error: ve } = await supabase.from("reel_battle_votes")
      .insert({ battle_id: battleId, participant_id: participantId, voter_id: user.id, vote_type: vt });
    if (ve) throw ve;
