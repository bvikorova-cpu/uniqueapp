...
import { handleFaceInsight } from "../_shared/faceInsight.ts";
...
serve(async (req) => {
  ...
  try {
    // Face Insight Studio is hosted inside this function (edge function slot limit).
    const peek = req.clone();
    const peeked = await peek.json().catch(() => ({} as any));
    if (peeked?.module === "face_insight") {
      return await handleFaceInsight(req, peeked);
    }
    ...
    const { action, prompt, age } = payload || {};
    if (typeof action !== "string" || !action) return json({ error: "Missing action" }, 400);
