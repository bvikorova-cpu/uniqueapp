...
const SYSTEM = `Analyze weekly nutrition. Return JSON: {summary, wins[], improvements[], next_week_focus[], score_0_100}.`;
...
serve(async (req) => {
  ...
  try {
    ...
    const creditDenied = await deductAICredits(user.id, 4, "nutrition-weekly-progress");
