...
serve(async (req) => {
  ...
  try {
    ...
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are a translator. Translate the given text to ${targetLanguage}. Only respond with the translation, nothing else.`
          },
    ...
    console.log(`User ${user.id} translated message, ${CREDIT_COST} credits deducted`);

    return new Response(JSON.stringify({ translation, creditsUsed: CREDIT_COST }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
