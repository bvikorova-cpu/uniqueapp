...
const Body = z.object({ action: z.enum(["save-pick", "submit-ticket"]),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await admin.from("lottery_saved_picks").insert({ user_id: userId,
      game_type: parsed.data.game_type,
      numbers: parsed.data.numbers,
      bonus_numbers: parsed.data.bonus_numbers ?? null,
      label: parsed.data.label ?? null,
      submitted: parsed.data.action === "submit-ticket" }).select().single();
