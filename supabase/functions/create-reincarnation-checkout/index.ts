...
serve(async (req) => {
  ...
  try {
    ...
    const mode = serviceType === "karmic_debt_calculator" || serviceType === "soulmate_matching" 
      ? "subscription" 
      : "payment";
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: mode,
      success_url: `${req.headers.get("origin")}/reincarnation-social?payment=success&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${req.headers.get("origin")}/reincarnation-social?payment=canceled`,
      metadata: {
        user_id: user.id,
