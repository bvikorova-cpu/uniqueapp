...
serve(async (req) => {
  ...
  try {
    ...
    const mode = serviceType === "fear_marketplace" || serviceType === "phobia_cure" || serviceType === "rare_fear_collector"
      ? "subscription" 
      : "payment";
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: mode,
      success_url: `${req.headers.get("origin")}/phobia-trading?payment=success&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${req.headers.get("origin")}/phobia-trading?payment=canceled`,
      metadata: {
        user_id: user.id,
