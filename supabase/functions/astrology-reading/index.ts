...
const SYSTEMS: Record<string, { system: string; json: boolean }> = {
  ...
  rune: { system: "You are a Norse rune reader. Draw 1 rune and interpret it in depth. Return JSON with EXACTLY these keys: {\"rune_name\":string, \"symbol\":string, \"meaning\":string (1-2 sentences, the short core meaning), \"description\":string (5-8 rich sentences: the rune's origin in the Elder Futhark, its symbolism, mythology and what it signals when drawn today), \"keywords\":[string] (4-6 single words), \"element\":string, \"deity\":string, \"guidance\":string (3-5 practical sentences of advice for the querent), \"reversed_meaning\":string (2-3 sentences), \"affirmation\":string}. English only.", json: true },
