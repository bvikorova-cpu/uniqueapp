...
serve(async (req) => {
  ...
  try {
    ...
    const { prompt, durationSeconds = 5, promptInfluence = 0.4 } = await req.json();
    ...
    if (!r.ok) {
      ...
      await refund();
      return new Response(JSON.stringify({ error: `ElevenLabs SFX error ${r.status}: ${txt.slice(0, 200)}` }), { status: 502, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    console.error('[video-ad-sfx]', msg);
