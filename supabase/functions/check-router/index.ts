  ...
  try {
    ...
    if (action === "anonymous_date_access") {
      ...
      if (hasActive) {
        ...
        if (existing) { await sb.from("anonymous_dating_subscriptions").update({
        ...
        } else { await sb.from("anonymous_dating_subscriptions").insert({
            user_id: user.id,
            stripe_customer_id: customerId,
            stripe_subscription_id: stripeSubscriptionId,
            subscription_status: "active",
      ...
      } else { await sb.from("anonymous_dating_subscriptions").update({
    ...
    if (connectActions.has(action)) {
      ...
      if (action === "live_status") {
        ...
        try {
          ...
          recentPayouts = list.data.map((p) => ({ id: p.id, amount: p.amount, currency: p.currency,
        ...
        await sb.from("profiles").update({ stripe_connect_charges_enabled: !!acct.charges_enabled,
          ...
          stripe_connect_synced_at: new Date().toISOString() }).eq("id", user.id);
        return json({ connected: true,
          ...
          balance: balance ? {
            ...
            instant_available: (balance as any).instant_available ?? null } : null,
      ...
      if (action === "update_payout_schedule") {
        ...
        await sb.from("profiles").update({ stripe_connect_payout_schedule: newSched,
          stripe_connect_synced_at: new Date().toISOString() }).eq("id", user.id);
      ...
      if (action === "start_stream" || action === "stop_stream") { try {
          await sb.from("profiles").update({
            is_streaming: action === "start_stream",
            last_stream_action_at: new Date().toISOString() }).eq("id", user.id);
        } catch { /* non-critical */ }
    ...
    if (action === "phobia") {
      ...
      return json({ subscribed, subscription_end,
    ...
    if (action === "megatalent_vip") {
      ...
      await sb.from("megatalent_vip_viewers").upsert({ user_id: user.id,
        status: isVip ? "active" : "inactive",
        stripe_customer_id: customerId,
        stripe_subscription_id: vipSub?.id ?? null,
        current_period_end: end }, { onConflict: "user_id" });
