  ...
  try {
    ...
    if (body.product === "pet" && body.priceId) {
      const PET_PRICES = new Set<string>([
        ...
        "price_1TpIITGaXSfGtYFtldmVP3Iv", // Single translation €2 (prod_UowA3EYrl5a8eu)
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [{
          price_data: {
            ...
            unit_amount: amt },
          quantity: 1 }],
        success_url: `${origin}/live/${streamId}?super_chat=success&session_id={CHECKOUT_SESSION_ID}`,
        cancel_url: `${origin}/live/${streamId}?super_chat=cancel`,
