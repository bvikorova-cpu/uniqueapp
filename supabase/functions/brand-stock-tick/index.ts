...
serve(async (req) => {
  ...
  try {
    ...
    for (const p of prices as any[]) { const totalVotes = p.brand?.total_votes ?? 0;
    ...
    for (const u of updates) { await admin.from("brand_stock_prices").update({
        current_price: u.current_price,
        high_24h: u.high_24h,
        low_24h: u.low_24h,
        change_24h: u.change_24h,
        volume_24h: u.volume_24h,
        updated_at: u.updated_at }).eq("id", u.id);
    }
