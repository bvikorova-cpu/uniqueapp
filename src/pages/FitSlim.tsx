                  ...
                  {myPlans.map((plan) => (
                    <Card key={plan.id} className="bg-card/50 border-border/50 hover:border-green-500/50 cursor-pointer transition-all" onClick={() => openExistingPlan(plan)}>
                      <CardContent className="p-4 space-y-2">
                        ...
                        {plan.status === "completed" && <Button variant="outline" size="sm" className="w-full mt-2 border-green-500/30 text-green-400 hover:bg-green-500/10" onClick={() => toast({ description: "View Plan — coming soon" })}>View Plan</Button>}
                      </CardContent>
                    </Card>
                  ))}
                </div>
