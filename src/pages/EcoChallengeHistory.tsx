...
export default function EcoChallengeHistory() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { data: lb } = await supabase.rpc("get_eco_leaderboard", { _month_key: selected, _limit: 50 });
      ...
      setLeaderboard(((lb || []) as any[]).map((r: any) => ({ ...r, profile: pmap.get(r.user_id) })));
      setLoading(false);
    })();
  }, [selected]);
    ...
    <div className="min-h-screen bg-gradient-to-b from-green-50 via-emerald-50 to-teal-50 dark:from-green-950/40 dark:via-emerald-950/40 dark:to-teal-950/40">
      <div className="container mx-auto px-4 py-6 max-w-4xl">
        <div className="flex items-center justify-between mb-4">
          <Link to="/eco-challenge">
            <Button variant="ghost" size="sm"><ArrowLeft className="w-4 h-4 mr-1" /> Back</Button>
          </Link>
          <Badge className="bg-emerald-600">🌱 Eco Challenge</Badge>
        </div>
