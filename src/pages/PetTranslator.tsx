        ...
        {!subscription.subscribed ? (
          ...
          <div className="space-y-8">
            <Card className="p-8 sm:p-12 text-center bg-gradient-to-br from-purple-500/10 to-fuchsia-500/10 border-purple-500/20">
              ...
              <p className="text-lg text-muted-foreground mb-8 max-w-md mx-auto">
                Subscribe to access 6 powerful AI tools, gamification, and weekly challenges
              </p>
          ...
          <Tabs defaultValue="tools" className="w-full">
            <TabsList className="w-full flex flex-wrap h-auto gap-1 bg-card/50 p-1 mb-6">
              {["tools", "streaks", "challenges", "leaderboard", "achievements"].map(tab => (
                <TabsTrigger key={tab} value={tab} className="flex-1 min-w-[80px] text-xs sm:text-sm capitalize">
                  {tab === "tools" ? "🧬 AI Tools" : tab === "streaks" ? "🔥 Streaks" : tab === "challenges" ? "🎯 Challenges" : tab === "leaderboard" ? "🏆 Leaderboard" : "🏅 Achievements"}
                </TabsTrigger>
              ))}
            </TabsList>
