  ...
  const copyMediaLink = async (url: string) => {
    try {
      ...
      toast({
        ...
        description: 'Link na médiá bol skopírovaný',
      });
    } catch (error) {
      toast({
        title: t('megatalent.error'),
                          ...
                          <DialogContent className="max-w-lg max-h-[80vh]">
                            ...
                            <ScrollArea className="h-[400px] pr-4">
                              {comments[submission.id]?.length > 0 ? (
                                ...
                                <p className="text-center text-muted-foreground py-8">Zatiaľ žiadne komentáre</p>
                              )}
                            </ScrollArea>
