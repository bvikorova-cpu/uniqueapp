...
const HIW_STEPS = [
  { title: "1. No daily obligation", desc: "You do NOT have to post every day, and there is not a new challenge every day. The only rule: add at least ONE photo or video of your healthy action between the first and the last day of each calendar month." },
  ...
  { title: "3. Submit your proof", desc: "Add a short description (min 10 chars), upload up to 4 photos or 1 video (≤50 MB). You can post on any day you want, maximum 1 submission per day." },
...
export default function HealthyChallenge() {
  ...
  const videoRefEl = useRef<HTMLInputElement>(null);
  ...
  const loadAll = async () => {
    ...
    if (!ch) { ch = {
        ...
        description: "Any activity: run, brisk walk, gym, bike, swim, yoga. Snap your watch showing time or distance as proof.",
        category: "fitness",
        icon: "🏃",
        xp_reward: 50,
    ...
    if (user) {
      ...
      const { data: monthSubs } = await (supabase as any)
        .from("healthy_submissions")
    ...
    const { data: lb } = await (supabase as any).rpc("get_healthy_leaderboard", { _month_key: currentMonthKey(), _limit: 20 });
    ...
    const { data: w } = await (supabase as any).from("healthy_monthly_winners").select("*").order("month_key", { ascending: false }).limit(12);
    setWinners(w || []);
  ...
  const uploadMedia = async (): Promise<{ images: string[]; video: string | null }> => {
    ...
    for (const f of files.slice(0, 4)) {
      const path = `${user.id}/healthy-${Date.now()}-${Math.random().toString(36).slice(2)}-${f.name.replace(/[^a-zA-Z0-9.\-]/g, "_")}`;
    ...
    if (videoFile) {
      ...
      const path = `${user.id}/healthy-${Date.now()}-video-${videoFile.name.replace(/[^a-zA-Z0-9.\-]/g, "_")}`;
  ...
  const toggleVote = async (sub: Submission) => {
    ...
    if (sub.hasVoted) {
      await (supabase as any).from("healthy_votes").delete().eq("submission_id", sub.id).eq("voter_id", user.id);
    } else {
      const { error } = await (supabase as any).from("healthy_votes").insert({ submission_id: sub.id, voter_id: user.id });
    ...
    <div className="min-h-screen bg-gradient-to-b from-orange-50 via-rose-50 to-sky-50 dark:from-orange-950/40 dark:via-rose-950/40 dark:to-sky-950/40">
      <div className="container mx-auto px-4 py-6 max-w-5xl">
        ...
        <div className="relative overflow-hidden rounded-3xl shadow-2xl mb-4 aspect-video">
          <video
            src={sectionVideos.healthyChallenge}
            autoPlay
            loop
            muted
            playsInline
        ...
        <div className="relative overflow-hidden rounded-2xl border-2 border-orange-300/30 bg-gradient-to-br from-rose-950 via-orange-900 to-amber-900 p-5 sm:p-6 shadow-xl mb-6">
          <Badge className="bg-orange-500/90 text-white border-orange-300/50 shadow-lg w-fit mb-3">
            💪 Global Healthy Challenge
          </Badge>
          ...
          <TabsContent value="today" className="space-y-4">
              ...
              <Card className="border-orange-200 dark:border-orange-900">
                <CardHeader>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <Badge className="bg-orange-600">+{challenge.xp_reward} XP</Badge>
                  </div>
            ...
            {user ? (
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Button type="button" variant="outline" size="sm" onClick={() => videoRefEl.current?.click()}>
                      ...
                      <input ref={videoRefEl} type="file" accept="video/*" hidden onChange={(e) => setVideoFile(e.target.files?.[0] || null)} />
                    </div>
                    {files.length > 0 && (
                    ...
                    <Button onClick={submit} disabled={uploading} className="w-full bg-orange-600 hover:bg-orange-700">
              ...
              <Card><CardContent className="pt-6 text-center">
                <Dumbbell className="w-10 h-10 mx-auto mb-2 text-orange-600" />
                <p className="mb-3">Sign in to submit your healthy proof.</p>
