...
import { Loader2, Sparkles, Heart, Brain, Wind, Palette, BookOpen, Check, Volume2, TrendingUp, Crown, Zap, Shield, Lock } from "lucide-react";
    ...
    <div className="container mx-auto px-3 sm:px-4 py-8 sm:py-16 pt-20 sm:pt-20 max-w-7xl">
      ...
      <div className="mb-8 sm:mb-12 space-y-4 sm:space-y-6">
        <div className="text-center max-w-4xl mx-auto">
          ...
          <motion.p
            ...
            className="text-sm sm:text-lg text-muted-foreground max-w-xl mx-auto"
          >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 mt-4 sm:mt-8">
          {[
            { icon: Brain, label: "AI Coach", desc: "Empathetic AI assistant for your mental wellbeing", color: "text-violet-400", bg: "from-violet-500/10 to-purple-500/5" },
            { icon: Wind, label: "Breathing", desc: "4-7-8 and Box Breathing with visual guidance", color: "text-sky-400", bg: "from-sky-500/10 to-cyan-500/5" },
            { icon: Heart, label: "Body Scan", desc: "Progressive relaxation with audio guidance", color: "text-rose-400", bg: "from-rose-500/10 to-pink-500/5" },
            { icon: Volume2, label: "Sounds", desc: "Rain, ocean waves, and forest for relaxation", color: "text-emerald-400", bg: "from-emerald-500/10 to-green-500/5" },
          ].map((item, index) => {
        ...
        {/* Info Section */}
        <Card className="relative overflow-hidden border-primary/20 backdrop-blur-xl bg-card/80 mt-6">
              ...
              <div className="border-t border-border/50 pt-4">
                ...
                <div className="grid sm:grid-cols-2 gap-2">
                  {[
                    ...
                    { step: "4", text: "Use 5-4-3-2-1 grounding to reduce anxiety" },
                    { step: "5", text: "Play nature sounds while working or sleeping" },
                    { step: "6", text: "Chat with AI Coach 24/7 (Premium)" },
                    { step: "7", text: "Write gratitude entries with AI insights (Premium)" },
                    { step: "8", text: "Track your progress over time" },
                  ].map((item) => (
              ...
              <div className="border-t border-border/50 pt-4">
                ...
                <div className="grid sm:grid-cols-2 gap-2">
                  {[
                    ...
                    { text: "Improve sleep quality", sub: "Body scan and nature sounds aid restful sleep" },
                    { text: "Enhanced focus", sub: "Grounding brings you to the present moment" },
      ...
      <Tabs defaultValue="breathing" className="w-full">
        <div className="overflow-x-auto mb-4">
          <TabsList className="inline-flex w-max min-w-full sm:grid sm:w-full sm:grid-cols-8 backdrop-blur-xl bg-card/60 border border-border/30">
            <TabsTrigger value="progress" disabled={!hasBasicAccess} className="text-xs px-2 data-[state=active]:shadow-lg">
              <TrendingUp className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
              <span className="hidden sm:inline">Progress</span>
            </TabsTrigger>
            <TabsTrigger value="breathing" disabled={!hasBasicAccess} className="text-xs px-2 data-[state=active]:shadow-lg">
              <Wind className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
              <span className="hidden sm:inline">Breathing</span>
            </TabsTrigger>
            <TabsTrigger value="grounding" disabled={!hasBasicAccess} className="text-xs px-2 data-[state=active]:shadow-lg">
              <Brain className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
              <span className="hidden sm:inline">Grounding</span>
            </TabsTrigger>
            <TabsTrigger value="sounds" disabled={!hasBasicAccess} className="text-xs px-2 data-[state=active]:shadow-lg">
              <Volume2 className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
              <span className="hidden sm:inline">Sounds</span>
            </TabsTrigger>
            <TabsTrigger value="bodyscan" disabled={!hasBasicAccess} className="text-xs px-2 data-[state=active]:shadow-lg">
              <Heart className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
              <span className="hidden sm:inline">Body Scan</span>
            </TabsTrigger>
            <TabsTrigger value="chat" disabled={!hasPremiumAccess} className="text-xs px-2 data-[state=active]:shadow-lg">
              <Brain className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
              <span className="hidden sm:inline">AI Coach</span>
              {!hasPremiumAccess && <Crown className="w-3 h-3 ml-0.5 text-amber-400" />}
            </TabsTrigger>
            <TabsTrigger value="journal" disabled={!hasPremiumAccess} className="text-xs px-2 data-[state=active]:shadow-lg">
              <BookOpen className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
              <span className="hidden sm:inline">Journal</span>
              {!hasPremiumAccess && <Crown className="w-3 h-3 ml-0.5 text-amber-400" />}
            </TabsTrigger>
            <TabsTrigger value="mandala" disabled={!hasPremiumAccess} className="text-xs px-2 data-[state=active]:shadow-lg">
