 ...
 */
export function handleEdgeError(err: AnyError, opts: EdgeErrorOptions = {}): boolean { const status = extractStatus(err);
  ...
  if (status === 401 || /unauthor/i.test(msg)) {
    toast.error(`Sign in required${ctx}`, { description: "You need to be signed in to use this feature.",
      action: {
        label: "Sign in",
  ...
  if (status === 402 || /insufficient credits|not enough credits/i.test(msg)) {
    toast.error(`Not enough credits${ctx}`, { description: "Top up your AI credits to continue.",
      action: {
        label: "Buy credits",
  ...
  if (status === 429 || /rate limit/i.test(msg)) { toast.error("Too many requests", {
