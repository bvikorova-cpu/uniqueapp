...
export function useAudioSpaces() {
  ...
  const schedule = useMutation({
    mutationFn: async (input: { title: string; description?: string; scheduled_at?: string | null }) => {
      ...
      const { error } = await (supabase as any).from("audio_spaces").insert({ host_id: u.user.id,
        title: input.title,
  ...
  const endSpace = useMutation({
    mutationFn: async ({ id, recording_url }: { id: string; recording_url?: string }) => {
      const { error } = await (supabase as any).from("audio_spaces").update({ status: "ended",
        ended_at: new Date().toISOString(),
        recording_url: recording_url ?? null }).eq("id", id);
      if (error) throw error;
    },
