...
import { Infinity, Sparkles, Play, Pause, Volume2, VolumeX, Globe, Crown, Layers } from "lucide-react";
...
export const MultiverseHero = () => {
  ...
  const heroStats = [
    ...
    { icon: Crown, label: "Platform Fee", value: 0, suffix: "", staticLabel: "€49+" },
  ];
  ...
  return (
    <div className="relative h-[78vh] min-h-[540px] w-full overflow-hidden rounded-3xl border border-border/40 mb-8">
      <video ref={videoRef} className="absolute inset-0 h-full w-full object-cover brightness-110 saturate-110" autoPlay muted loop playsInline>
        <source src="/__l5e/assets-v1/5bfcf50b-a751-4bf8-b2a1-3c01fc06b61a/multiverse-hero.mp4" type="video/mp4" />
      </video>
      ...
      <div className="relative z-10 h-full flex flex-col justify-center px-6 sm:px-10">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex justify-center mb-4">
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-card/45 backdrop-blur-md text-foreground text-sm font-semibold border border-border/60">
            <Infinity className="w-4 h-4 text-primary" /> Infinite Realities · AI-Generated Worlds <Sparkles className="w-4 h-4 text-accent" />
          </span>
        </motion.div>
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="text-3xl md:text-5xl lg:text-6xl font-black text-center mb-3 bg-gradient-to-r from-foreground via-primary to-accent bg-clip-text text-transparent drop-shadow-2xl">
          Multiverse Profile Network
        </motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
          className="text-sm sm:text-base md:text-lg text-foreground/85 text-center mb-7 max-w-3xl mx-auto">
          One user, infinite versions across parallel worlds. Explore alternate realities and discover your best self.
        </motion.p>
