...
export function TeamBuilder({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("hockey_teams").select("*").eq("user_id", user.id).single().then(({ data }) => { setTeam(data); if (data) { setTeamName(data.name); setPlaystyle(data.playstyle); } });
  ...
  const saveTeam = async () => {
    ...
    if (team) {
      await supabase.from("hockey_teams").update({ name: teamName, playstyle }).eq("id", team.id);
    } else {
      const { data } = await supabase.from("hockey_teams").insert({ user_id: user.id, name: teamName, playstyle }).select().single();
      setTeam(data);
    }
  ...
  const toggleStarter = async (player: any) => {
    ...
    if (!player.is_starter && starters.length >= 6) { toast.error("Max 6 starters (5 skaters + 1 goalie)!"); return; }
    await supabase.from("hockey_players").update({ is_starter: !player.is_starter }).eq("id", player.id);
