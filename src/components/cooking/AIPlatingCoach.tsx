...
export default function AIPlatingCoach({ onBack }: Props) {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("generate-gift-message", {
        body: { type: "cooking_ai", prompt: `You are a Michelin-star chef specializing in food presentation and plating. For the dish described, provide: 1) Plate selection (shape, color, size), 2) Color composition strategy, 3) Step-by-step plating instructions, 4) Sauce placement techniques (dots, swoosh, pool), 5) Garnish recommendations (micro greens, edible flowers, etc.), 6) Height and dimension tips, 7) Photography angle suggestions for Instagram. Dish: ${input}` } });
  ...
  return (
    <>
      <FloatingHowItWorks title="How AIPlating Coach works" steps={[
