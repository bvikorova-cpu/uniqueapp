...
export const HorseRacingHero = ({ stats, onNavigate }: HorseRacingHeroProps) => {
  ...
  return (
    <>
      ...
      <section className="relative h-[70svh] min-h-[480px] overflow-hidden rounded-2xl mx-2 md:mx-0 bg-amber-50">
      {/* Video */}
      ...
      <div className="absolute top-4 right-16 w-12 h-12 border-r-2 border-t-2 border-amber-500/60 rounded-tr-lg" />
      <div className="absolute bottom-4 left-4 w-12 h-12 border-l-2 border-b-2 border-amber-500/60 rounded-bl-lg" />
      ...
      <div className="absolute top-4 right-4 z-20 flex gap-2">
        <Button size="icon" variant="ghost" onClick={togglePlay}
          className="h-9 w-9 rounded-full bg-amber-50/80 backdrop-blur-sm text-amber-700 hover:bg-amber-50/90 border border-amber-300/60">
      ...
      <div className="absolute inset-0 z-10 flex flex-col justify-end p-4 md:p-10 pb-6 md:pb-10">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          <div className="inline-flex items-center gap-2 mb-3 px-4 py-1.5 bg-amber-500/15 backdrop-blur-sm rounded-full border border-amber-400/60">
            <div className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
            <span className="text-amber-700 font-mono text-xs uppercase tracking-[0.2em]">
          ...
          <p className="text-amber-700/70 text-sm md:text-lg max-w-xl mb-6 leading-relaxed font-mono">
            Breed champions. Dominate the track. Claim the trophy.
          </p>

          <div className="flex gap-3 mb-6">
            <Button onClick={() => onNavigate("stable")} size={isMobile ? "default" : "lg"}
              className="bg-gradient-to-r from-amber-600 to-red-600 hover:from-amber-500 hover:to-red-500 text-white font-mono uppercase tracking-wider border border-amber-400/60 shadow-lg shadow-amber-500/30">
              🏇 Enter Arena
            </Button>
            {!user && (
              <Button onClick={() => navigate("/auth")} variant="outline" size={isMobile ? "default" : "lg"}
                className="border-amber-400/60 text-amber-700 bg-amber-50/70 hover:bg-amber-50/80 backdrop-blur-sm font-mono uppercase tracking-wider">
                Sign In
              </Button>
            )}
          </div>
        ...
        >
          {statItems.map((stat, i) => (
            <div key={i} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-amber-50/80 backdrop-blur-md border border-amber-300/60">
              <div className="relative">
                <stat.icon className={`h-5 w-5 ${stat.color}`} />
                <div className={`absolute -inset-2 rounded-full blur-md opacity-30 ${stat.color.replace('text-', 'bg-')}`} />
              </div>
              <div>
                <p className="font-mono font-bold text-slate-900 text-sm md:text-lg">
                  {stats[stat.key] === 0 ? "—" : stats[stat.key].toLocaleString()}
                </p>
                <p className="text-[10px] font-mono text-amber-700/70 uppercase tracking-wider">{stat.label}</p>
              </div>
            </div>
          ))}
        </motion.div>
