...
const TIERS = [
  { id: "bronze" as const, name: "Bronze", price: "€4.99/mo", perks: ["Patron badge", "Exclusive stories"] },
  { id: "silver" as const, name: "Silver", price: "€9.99/mo", perks: ["All Bronze perks", "Voice narrations", "Early access"], popular: true },
  { id: "gold" as const, name: "Gold", price: "€19.99/mo", perks: ["All Silver perks", "Custom requests", "Direct messages"] },
];
        ...
        {TIERS.map((t) => (
          <div
            ...
            className={`p-4 rounded-xl border-2 bg-card/40 ${
              t.popular ? "border-yellow-700/60 shadow-[0_0_25px_rgba(180,140,0,0.3)]" : "border-yellow-900/30"
            }`}
          >
            {t.popular && <span className="block text-[10px] font-bold text-yellow-400 mb-1">POPULAR</span>}
            <p className="font-bold text-yellow-100">{t.name}</p>
            <p className="text-lg font-black text-yellow-200 mb-2">{t.price}</p>
            ...
            <Button
              ...
              onClick={() => checkout.mutate({ authorUserId, tier: t.id })}
