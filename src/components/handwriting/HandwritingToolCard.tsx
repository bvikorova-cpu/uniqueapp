...
interface HandwritingToolCardProps {
  tool: ToolDef;
  onSelect: () => void;
  index: number;
}

export const HandwritingToolCard = ({ tool, onSelect, index }: HandwritingToolCardProps) => {
  const Icon = tool.icon;

  return (
    <>
      <FloatingHowItWorks title={"Handwriting Tool Card - How it works"} steps={[{ title: 'Open', desc: 'Access the Handwriting Tool Card section from its module.' }, { title: 'Explore', desc: 'Review the controls and content available in Handwriting Tool Card.' }, { title: 'Interact', desc: 'Use the available actions - browse, select, or submit as needed.' }, { title: 'Review', desc: 'Check the results, updates, or feedback shown after your action.' }]} />
      <motion.div
    ...
    >
      <Card className="relative overflow-hidden group transition-all duration-300 hover:scale-[1.02] hover:shadow-xl hover:shadow-primary/5 hover:border-primary/30 bg-card/60 backdrop-blur-sm h-full">
        ...
        <div className={`pointer-events-none absolute inset-0 bg-gradient-to-br ${tool.color} opacity-0 group-hover:opacity-5 transition-opacity duration-300`} />
