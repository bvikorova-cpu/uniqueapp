    ...
    <div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h2 className="text-2xl font-black">AI Coupon Valuator</h2>
            <p className="text-muted-foreground text-sm">Get fair market price & risk analysis · 3 CR</p>
          </div>
          <Badge className="ml-auto bg-gradient-to-r from-purple-600 to-amber-500 text-white border-0 shadow-md">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="border-purple-500/10">
          ...
          <CardContent className="space-y-4">
            ...
            <div><label className="text-sm font-semibold mb-1.5 block">Your Asking Price (€)</label><Input type="number" placeholder="e.g., 35" value={askingPrice} onChange={e => setAskingPrice(e.target.value)} /></div>
            ...
            <Button onClick={generate} disabled={loading} className="w-full h-11 bg-gradient-to-r from-purple-600 to-amber-500 hover:from-purple-700 hover:to-amber-600 shadow-lg">
        ...
        <Card className={result ? "border-purple-500/20" : ""}>
          ...
          <CardContent>{result ? <div className="prose prose-sm dark:prose-invert max-w-none whitespace-pre-wrap text-sm bg-muted/50 rounded-xl p-5 border">{result}</div> : <p className="text-muted-foreground text-sm text-center py-12">Enter coupon details to get a fair market valuation</p>}</CardContent>
        </Card>
      </div>
    </div>
  );
