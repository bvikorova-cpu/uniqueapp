...
export function CouponChatDialog({ open, onOpenChange, couponId, couponTitle, otherId, otherName }: Props) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`couponmsg-${couponId}-${otherId}`)
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "coupon_messages", filter: `coupon_id=eq.${couponId}` },
        (payload) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[560px] flex flex-col h-[600px]">
        <DialogHeader>
          ...
          <DialogDescription className="truncate">{couponTitle}</DialogDescription>
        </DialogHeader>
...
export default CouponChatDialog;
