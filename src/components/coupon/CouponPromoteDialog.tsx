...
export function CouponPromoteDialog({
  couponId,
  open,
  onOpenChange,
  onPromoted,
  ...
  const promote = async (tier: "top" | "premium", days: number) => {
    ...
    setBusy(`${tier}-${days}`);
    try {
      const { data, error } = await (supabase as any).rpc("coupon_top_listing", {
        _coupon_id: couponId,
        _days: days,
        _tier: tier,
      });
      ...
      toast({
        ...
        description: msg.includes("INSUFFICIENT_CREDITS")
          ...
          : msg.includes("NOT_OWNER")
            ? "You can promote only your own listing."
            : msg || "Try again",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Promote this coupon</DialogTitle>
          <DialogDescription>
            TOP coupons appear above standard ones. PREMIUM coupons get their own showcase folder at the very top.
          </DialogDescription>
        </DialogHeader>
          ...
          <Card className="border-primary/40">
            <CardContent className="p-4 space-y-3">
              ...
              <Button
                className="w-full"
                disabled={!!busy}
                onClick={() => promote("premium", PREMIUM.days)}
              >
...
export default CouponPromoteDialog;
