...
export const FootballArenaHero = ({ stats, onNavigate }: FootballArenaHeroProps) => {
  ...
  const statItems = [
    { icon: Users, label: "Players", value: stats.totalPlayers, color: "from-emerald-400 to-green-500", glow: "shadow-emerald-500/30" },
    ...
    { icon: Wifi, label: "Online", value: stats.onlineManagers, color: "from-emerald-400 to-cyan-500", glow: "shadow-emerald-500/30" },
  ];

  return (
    <div className="relative rounded-2xl overflow-hidden border border-emerald-500/20 shadow-2xl shadow-emerald-900/20">
      <div className="relative h-[320px] md:h-[420px]">
        <div className="absolute inset-0">
          <Suspense fallback={<div className="w-full h-full bg-gradient-to-br from-emerald-950 via-green-950 to-slate-950" />}>
            <Stadium3D sport="football" />
          </Suspense>
        </div>
        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent" />
        <div className="absolute inset-0 bg-gradient-to-r from-emerald-900/30 via-transparent to-green-900/20" />

        <motion.div className="absolute top-8 right-8 w-3 h-3 rounded-full bg-emerald-400/60 blur-[1px]"
        ...
        <div className="relative z-10 h-full flex flex-col justify-end p-5 md:p-8">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <motion.div className="inline-flex items-center gap-2 mb-3" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}>
              <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/20 backdrop-blur-md border border-emerald-400/30">
                <Flame className="w-3 h-3 text-emerald-400" />
                <span className="text-[10px] font-bold text-emerald-300 tracking-widest uppercase">Season Active</span>
              </div>
            </motion.div>
            <div className="relative mb-2">
              <h1 className="text-3xl md:text-6xl font-black tracking-tight">
                <span className="bg-gradient-to-r from-emerald-300 via-green-200 to-emerald-400 bg-clip-text text-transparent drop-shadow-lg">Football</span>
                <br />
                <span className="text-white/95" style={{ textShadow: "0 2px 30px rgba(16,185,129,0.4)" }}>Arena</span>
              </h1>
              <motion.div className="absolute -left-1 top-0 w-1 h-full bg-gradient-to-b from-emerald-500 via-green-400 to-transparent rounded-full"
            ...
            <p className="text-sm md:text-base text-white/70 max-w-md font-medium leading-relaxed">
              Build your dream team, dominate the league, and claim eternal glory
            </p>
          </motion.div>
        </div>
      </div>

      <div className="grid grid-cols-4 gap-2 md:gap-3 p-3 md:p-4 bg-gradient-to-b from-slate-900/95 to-slate-950/95 backdrop-blur-xl border-t border-emerald-500/10">
