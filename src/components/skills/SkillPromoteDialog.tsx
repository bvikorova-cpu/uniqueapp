...
export function SkillPromoteDialog({
  offeringId,
  open,
  onOpenChange,
  onPromoted,
  ...
  const promote = async (tier: "top" | "premium", days: number) => {
    setBusy(`${tier}-${days}`);
    try {
      const { data, error } = await (supabase as any).rpc("skill_top_listing", {
      ...
      toast({
        ...
        description: msg.includes("INSUFFICIENT_CREDITS")
          ...
          : msg.includes("NOT_OWNER")
            ? "You can promote only your own offering."
            : msg || "Try again",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Promote this offering</DialogTitle>
          <DialogDescription>
            TOP offerings appear above standard ones. PREMIUM offerings get their own folder at the very top.
          </DialogDescription>
        </DialogHeader>
