 ...
 */
export function MedicalTrustBadges({ verified,
  directToHospital,
  refundGuarantee,
  medicalDocumentsCount = 0 }: Props) { const badges = [
    {
      show: !!verified,
      icon: ShieldCheck,
  ...
  const colorClass = (c: string) =>
    ({ emerald: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/30",
      ...
      amber: "bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/30" }[c] || "bg-muted text-foreground border-border");
