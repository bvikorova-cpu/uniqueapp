...
import { Heart, PawPrint, TrendingUp, ShieldCheck } from "lucide-react";
...
const floatingEmojis = [
  ...
  { emoji: "❤️", x: "92%", y: "42%", delay: 2.5, duration: 7 },
  { emoji: "🏥", x: "5%", y: "48%", delay: 3, duration: 5 },
];

const stats = [
  { icon: PawPrint, label: "Animals Helped", value: "—" },
  { icon: ShieldCheck, label: "Shelters Verified", value: "—" },
    ...
    <div className="relative overflow-hidden py-16 md:py-24">
      {floatingEmojis.map((item, i) => (
        <motion.div
          ...
          animate={{ y: [0, -18, 0, 18, 0], x: [0, 8, -8, 4, 0], rotate: [0, 8, -8, 4, 0], scale: [1, 1.1, 0.9, 1.05, 1] }}
      ...
      <div className="relative z-10 text-center max-w-3xl mx-auto px-4">
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}
          ...
          <span className="text-sm font-medium text-primary">Every Pet Deserves Love</span>
        </motion.div>
        ...
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          Help animals in need get the medical treatment and loving homes they deserve.
        </motion.p>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
          <Button size="lg" onClick={() => navigate("/fundraising/pet/create")}
            className="bg-gradient-to-r from-primary to-accent hover:opacity-90 text-white shadow-lg text-lg px-8">
            <Heart className="mr-2 h-5 w-5" />
            Help an Animal
          </Button>
          <Button size="lg" variant="outline" onClick={() => document.getElementById("pet-campaigns")?.scrollIntoView({ behavior: "smooth" })}>
            Browse Cases
          </Button>
        </motion.div>
