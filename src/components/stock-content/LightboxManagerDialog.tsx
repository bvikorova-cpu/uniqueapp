...
export function LightboxManagerDialog({ open, onOpenChange, contentItemId }: LightboxManagerDialogProps) {
  ...
  const createLightbox = async () => {
    if (!name.trim()) { toast({ title: "Zadaj názov", variant: "destructive" }); return; }
    setCreating(true);
    const { data: { user } } = await supabase.auth.getUser();
    ...
    toast({ title: "Lightbox vytvorený" });
  ...
  const addToLightbox = async (lightboxId: string) => {
    ...
    toast({ title: "Pridané do lightboxu" });
    load();
  };
  ...
  const removeFromLightbox = async (lightboxId: string) => {
    ...
    toast({ title: "Odstránené z lightboxu" });
    load();
  };

  const deleteLightbox = async (id: string) => {
    if (!confirm("Vymazať lightbox?")) return;
    await supabase.from("stock_lightboxes").delete().eq("id", id);
    toast({ title: "Lightbox vymazaný" });
    load();
  };
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {contentItemId ? "Pridať do Lightboxu" : "Moje Lightboxy"}
          </DialogTitle>
          <DialogDescription>
            Organizuj a uchovávaj inšpiratívne diela do vlastných kolekcií.
          </DialogDescription>
        </DialogHeader>

        <Card className="p-4 space-y-3 border-primary/30">
          <div className="flex items-center gap-2 font-semibold"><Plus className="w-4 h-4" /> Nový lightbox</div>
          <Input placeholder="Názov..." value={name} onChange={(e) => setName(e.target.value)} />
          <Textarea placeholder="Popis (voliteľné)..." value={description} onChange={(e) => setDescription(e.target.value)} rows={2} />
        ...
        <div className="space-y-2">
          {loading ? (
            <div className="text-center py-6 text-muted-foreground text-sm">Načítavam...</div>
          ) : lightboxes.length === 0 ? (
            <div className="text-center py-6 text-muted-foreground text-sm">Žiadne lightboxy. Vytvor prvý vyššie.</div>
          ) : (
            lightboxes.map((lb) => (
              <Card key={lb.id} className="p-3 flex items-center gap-3">
                ...
                {contentItemId ? (
                  lb.contains_item ? (
                    <Button size="sm" variant="outline" onClick={() => removeFromLightbox(lb.id)}>Odstrániť</Button>
                  ) : (
                    <Button size="sm" onClick={() => addToLightbox(lb.id)}>Pridať</Button>
                  )
                ) : (
