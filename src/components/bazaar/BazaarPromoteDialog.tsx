...
export function BazaarPromoteDialog({
  itemId,
  open,
  onOpenChange,
  onPromoted,
  ...
  const promote = async (tier: "top" | "premium", days: number) => {
    ...
    setBusy(`${tier}-${days}`);
    try {
      const { data, error } = await (supabase as any).rpc("bazaar_top_listing", {
        _item_id: itemId,
        _days: days,
        _tier: tier,
      });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            TOP listings appear above standard ones. PREMIUM listings get their own showcase folder at the very top.
          </DialogDescription>
        </DialogHeader>
...
export default BazaarPromoteDialog;
