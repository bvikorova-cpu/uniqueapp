...
export function PriceEstimatorView({ onBack }: Props) {
  ...
  const generate = async () => {
    if (!title.trim()) { toast.error("Enter item title"); return; }
    setLoading(true);
    try {
      ...
      const { data, error } = await supabase.functions.invoke("bazaar-ai", {
        body: { action: "price-estimator", title, category, condition, description, askingPrice },
  ...
  return (
    <>
      <FloatingHowItWorks title="How Price Estimator View works" steps={[
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h2 className="text-2xl font-black">AI Price Estimator</h2>
            <p className="text-muted-foreground text-sm">Get a fair market price for any item · 3 CR</p>
          </div>
        ...
        <Card className="border-amber-500/10">
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Input placeholder="e.g., iPhone 14 Pro 128GB" value={title} onChange={e => setTitle(e.target.value)} />
            </div>
            <div>
            ...
            <div>
              ...
              <Textarea placeholder="Describe the item in detail..." value={description} onChange={e => setDescription(e.target.value)} rows={3} />
            </div>
            <div>
              <label className="text-sm font-semibold mb-1.5 block">Your Asking Price (€)</label>
              <Input type="number" placeholder="e.g., 450" value={askingPrice} onChange={e => setAskingPrice(e.target.value)} />
            </div>
            <Button onClick={generate} disabled={loading} className="w-full h-11 bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 shadow-lg">
              {loading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Analyzing...</> : <><DollarSign className="w-4 h-4 mr-2" />Get Price Estimate (3 CR)</>}
            </Button>
          </CardContent>
        </Card>
        <Card className={result ? "border-amber-500/20" : ""}>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle>{result ? "Price Analysis" : "Results"}</CardTitle>
            {result && (
          ...
          <CardContent>
            {result ? (
              ...
              <p className="text-muted-foreground text-sm text-center py-12">Enter item details to get a fair market price estimate</p>
            )}
          </CardContent>
        </Card>
      </div>
