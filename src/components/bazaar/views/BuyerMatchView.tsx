...
export function BuyerMatchView({ onBack }: Props) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("bazaar-ai", {
        body: { action: "buyer-match", title, category, condition, description, price },
        headers: { Authorization: `Bearer ${session.access_token}` },
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
      setResult(data.result);
      toast.success(`Buyer profiles ready! (${data.credits_used} credits used)`);
  ...
  return (
    <>
      <FloatingHowItWorks title="How Buyer Match View works" steps={[
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h2 className="text-2xl font-black">AI Buyer Match</h2>
            <p className="text-muted-foreground text-sm">Find ideal buyers for your item · 4 CR</p>
          </div>
        ...
        <Card className="border-yellow-500/10">
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Input placeholder="e.g., Vintage Leather Jacket" value={title} onChange={e => setTitle(e.target.value)} />
            </div>
            <div>
            ...
            <div>
              ...
              <Input type="number" placeholder="e.g., 120" value={price} onChange={e => setPrice(e.target.value)} />
            </div>
            <Button onClick={generate} disabled={loading} className="w-full h-11 bg-gradient-to-r from-yellow-500 to-amber-600 hover:from-yellow-600 hover:to-amber-700 shadow-lg">
              {loading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Matching...</> : <><Target className="w-4 h-4 mr-2" />Find Buyers (4 CR)</>}
            </Button>
          </CardContent>
        </Card>
        <Card className={result ? "border-yellow-500/20" : ""}>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle>{result ? "Buyer Profiles" : "Results"}</CardTitle>
            {result && (
          ...
          <CardContent>
            {result ? (
              ...
              <p className="text-muted-foreground text-sm text-center py-12">Enter item details to find your ideal buyer profiles</p>
            )}
          </CardContent>
        </Card>
      </div>
