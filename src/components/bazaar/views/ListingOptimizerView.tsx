...
export function ListingOptimizerView({ onBack }: Props) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("bazaar-ai", {
        body: { action: "listing-optimizer", title, description, category, condition, price },
        headers: { Authorization: `Bearer ${session.access_token}` },
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
      setResult(data.result);
      toast.success(`Listing optimized! (${data.credits_used} credits used)`);
  ...
  return (
    <>
      <FloatingHowItWorks title="How Listing Optimizer View works" steps={[
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h2 className="text-2xl font-black">AI Listing Optimizer</h2>
            <p className="text-muted-foreground text-sm">Rewrite your listing for maximum sales conversion · 3 CR</p>
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="border-orange-500/10">
          <CardHeader><CardTitle>Current Listing</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div>
              <label className="text-sm font-semibold mb-1.5 block">Title *</label>
              <Input placeholder="Your current listing title" value={title} onChange={e => setTitle(e.target.value)} />
            </div>
            <div>
              <label className="text-sm font-semibold mb-1.5 block">Description</label>
              <Textarea placeholder="Your current description..." value={description} onChange={e => setDescription(e.target.value)} rows={4} />
            </div>
            <div>
            ...
            <div>
              ...
              <Input type="number" placeholder="e.g., 99" value={price} onChange={e => setPrice(e.target.value)} />
            </div>
            <Button onClick={generate} disabled={loading} className="w-full h-11 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 shadow-lg">
              {loading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Optimizing...</> : <><Wand2 className="w-4 h-4 mr-2" />Optimize Listing (3 CR)</>}
            </Button>
          </CardContent>
        </Card>
        <Card className={result ? "border-orange-500/20" : ""}>
          ...
          <CardContent>
            {result ? (
              ...
              <p className="text-muted-foreground text-sm text-center py-12">Paste your current listing to get an optimized version</p>
            )}
          </CardContent>
        </Card>
      </div>
