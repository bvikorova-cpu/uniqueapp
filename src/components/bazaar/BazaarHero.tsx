...
export const BazaarHero = ({ itemCount }: BazaarHeroProps) => {
  ...
  const statCards = [
    { icon: Package, label: "Active Listings", value: stats.items.toLocaleString(), color: "from-amber-500 to-orange-600" },
    { icon: Users, label: "Verified Sellers", value: stats.sellers.toLocaleString(), color: "from-orange-500 to-red-500" },
    { icon: ShoppingCart, label: "Completed Orders", value: stats.orders.toLocaleString(), color: "from-yellow-500 to-amber-600" },
    { icon: TrendingUp, label: "Satisfaction", value: `${stats.satisfaction}%`, color: "from-emerald-500 to-teal-600" },
  ];

  return (
    <div className="relative w-full h-[420px] sm:h-[480px] rounded-2xl overflow-hidden mb-8">
      <video
