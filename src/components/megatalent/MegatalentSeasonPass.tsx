    ...
    <Card className="overflow-hidden backdrop-blur-xl bg-gradient-to-br from-primary/15 via-accent/10 to-primary/5 border-primary/20">
      <CardContent className="p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <Badge variant="secondary" className="ml-auto gap-1"><Sparkles className="h-3 w-3" /> {xp} XP</Badge>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {TIERS.map(t => {
            ...
            return (
              <motion.div key={t.lvl} whileHover={{ y: -2 }}
                className={`min-w-[110px] rounded-xl p-3 text-center border-2 transition ${
                  isCurrent ? "border-primary bg-primary/15" : unlocked ? "border-yellow-500/40 bg-yellow-500/5" : "border-border/30 bg-background/30 opacity-60"
                }`}>
                <div className="text-2xl mb-1">{unlocked ? t.icon : <Lock className="h-5 w-5 mx-auto text-muted-foreground" />}</div>
                ...
                <div className="text-[10px] text-muted-foreground mt-1 flex items-center justify-center gap-0.5"><Star className="h-2.5 w-2.5" />{t.xp}</div>
              </motion.div>
            );
          })}
        </div>
