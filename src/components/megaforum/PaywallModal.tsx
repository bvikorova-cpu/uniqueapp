...
interface PaywallModalProps {
  open: boolean;
  onClose: () => void;
  action?: string; // e.g. "post", "like", "comment"
}

export const PaywallModal = ({ open, onClose, action = "interact" }: PaywallModalProps) => {
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-sm bg-card/95 backdrop-blur-xl border-primary/20">
        <DialogHeader>
          ...
          <DialogTitle className="text-center text-lg">Join the conversation</DialogTitle>
          <DialogDescription className="text-center text-sm">
            To {action} on Megaforum you need an account.
            Browsing stays free — sign in only when you want to engage.
          </DialogDescription>
        </DialogHeader>
