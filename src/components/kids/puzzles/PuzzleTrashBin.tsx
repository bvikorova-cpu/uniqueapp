...
import { KidsPuzzle, pieceStyle } from "@/data/kidsPuzzles";
...
/** Scrap box for discarded puzzle pieces — 10 pieces = +1 AI credit. */
export const PuzzleTrashBin = ({ puzzle }: Props) => {
  ...
  const { data: rows = [], isLoading } = useQuery({
    queryKey: ["puzzle-trash", slug],
    queryFn: async () => {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return [] as TrashRow[];
      const { data, error } = await (supabase as any)
        .from("puzzle_piece_trash")
        .select("id, puzzle_slug, piece_index, created_at")
        .eq("user_id", user.id)
        .eq("puzzle_slug", slug)
  ...
  const toggle = (id: string) => {
    setSelected((prev) => {
      ...
      if (prev.length >= RECYCLE_BATCH) {
        toast.info(`You can recycle exactly ${RECYCLE_BATCH} pieces at a time.`);
        return prev;
      }
      return [...prev, id];
    });
  ...
  const recycle = async () => {
    ...
    setRecycling(true);
    try {
      const { data, error } = await (supabase as any).rpc("puzzle_trash_recycle", { _trash_ids: selected });
      if (error) throw error;
      toast.success(`${RECYCLE_BATCH} pieces recycled — +1 AI credit added (balance ${data?.balance ?? "updated"}).`);
      setSelected([]);
    ...
    <div className="space-y-4">
      <Card className="p-4 border-border/30 bg-card/80 backdrop-blur-xl">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="min-w-0">
            <h3 className="font-black text-base">Scrap box</h3>
            <p className="text-xs text-muted-foreground">
              Pieces you sent here with ✗ stay in the scrap box. Pick exactly {RECYCLE_BATCH} and recycle them for +1 AI credit — they are destroyed permanently.
            </p>
          </div>
          <Badge variant="outline" className="ml-auto border-border/40">{rows.length} in box</Badge>
        </div>
        ...
        {rows.length < RECYCLE_BATCH && (
          <p className="text-[11px] text-muted-foreground mt-2">
            You need at least {RECYCLE_BATCH} scrapped pieces to recycle ({RECYCLE_BATCH - rows.length} more to go).
          </p>
        )}
      </Card>
      ...
      {rows.length === 0 ? (
        <Card className="p-8 text-center border-dashed border-border/40 bg-card/70">
          <p className="font-bold mb-1">Your scrap box is empty</p>
          <p className="text-xs text-muted-foreground">Pieces you reject with ✗ land here instead of disappearing.</p>
        </Card>
      ) : (
            ...
            return (
              ...
              >
                <div className="relative aspect-square">
                  <div
                    className="absolute inset-0 saturate-[0.7] opacity-85"
                    style={pieceStyle(puzzle, r.piece_index)}
                    role="img"
                    aria-label={`Scrapped puzzle piece number ${r.piece_index + 1}`}
                  />
                  {isSel && (
                ...
                <div className="p-2 text-center">
                  <p className="text-[11px] font-bold">#{r.piece_index + 1}</p>
                </div>
              </Card>
            );
          })}
