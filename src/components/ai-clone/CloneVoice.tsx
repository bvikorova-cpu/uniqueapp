  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("clone-chat", {
        body: {
          message: `Rewrite this message in a ${voiceStyle} voice style, keeping the same meaning but adjusting tone and word choice: "${text}"`,
          cloneId: "voice-style",
          clonePersonality: `You are a voice style adapter. Transform text into the requested tone. Style: ${voiceStyle}. Return only the rewritten text.`
        }
      });
