...
export function AuctionChatDialog({ open, onOpenChange, auctionId, auctionTitle, otherId, otherName, initialMessage }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async (initial = false) => {
      ...
      if (cancelled) return;
      const rows = (((data as any[]) || []) as Msg[]).filter(
        (m) =>
    ...
    const channel = supabase
      .channel(`auctionmsg-${auctionId}-${otherId}`)
      .on(
        "postgres_changes",
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[600px] flex-col sm:max-w-[560px]">
        ...
        <ScrollArea className="flex-1 rounded-md border p-3" ref={scrollRef as any}>
          {loading ? (
