    ...
    <div className="space-y-6">
      <Card className="relative overflow-hidden border-border/30 bg-card/90 backdrop-blur-xl p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <Button className="bg-gradient-to-r from-yellow-600 to-amber-600 hover:from-yellow-700 hover:to-amber-700 text-white font-bold">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                ...
                <div className="shrink-0">
                  {tournament.status === "registration" && (
                    <Button className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-bold w-full sm:w-auto">
                      <Swords className="mr-2 h-4 w-4" /> Join ({tournament.entry_fee} cr)
                    </Button>
                  )}
                  {tournament.status === "in_progress" && (
                    <Button variant="outline" className="border-blue-500/30 text-blue-400 w-full sm:w-auto">View Bracket</Button>
                  )}
                  {tournament.status === "completed" && (
                    <Button variant="outline" className="border-amber-500/30 text-amber-400 w-full sm:w-auto">View Results</Button>
                  )}
                </div>
              </div>
            </Card>
