...
export function LeagueSystem({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    supabase.from("american_football_leagues").select("*").order("created_at", { ascending: false }).then(({ data }) => setLeagues(data || []));
  }, []);
  ...
  const joinLeague = async (league: any) => {
    ...
    if (league.entry_fee > 0) {
      ...
      await supabase.from("american_football_coins").update({ balance: coins.balance - league.entry_fee, total_spent: coins.total_spent + league.entry_fee }).eq("user_id", user.id);
    }
    await supabase.from("american_football_league_standings").insert({ league_id: league.id, team_id: team.id });
...
<div className="space-y-4">
      ...
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><Trophy className="h-5 w-5 text-primary" />Football Leagues</CardTitle></CardHeader>
        <CardContent>
