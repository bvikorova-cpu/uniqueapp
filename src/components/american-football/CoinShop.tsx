...
export function CoinShop({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("american_football_coins").select("balance").eq("user_id", user.id).single().then(({ data }) => setBalance(data?.balance ?? 0));
  }, [user]);
  ...
  const purchase = async (pack: typeof COIN_PACKS[0]) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("create-checkout", {
        ...
        body: { priceId: pack.priceId, productKey: "af_coins", metadata: { coins: pack.coins, module: "american_football" } },
      });
