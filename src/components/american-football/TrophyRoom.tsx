...
export function TrophyRoom({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    if (!user) return;
    Promise.all([
      supabase.from("american_football_teams").select("*").eq("user_id", user.id).single(),
      supabase.from("american_football_players").select("id", { count: "exact", head: true }).eq("user_id", user.id),
  ...
  const trophies = [
    { name: "First Win", desc: "Win your first game", earned: stats.wins >= 1, icon: "🏆" },
    { name: "Five Wins", desc: "Win 5 games", earned: stats.wins >= 5, icon: "🥇" },
    { name: "Squad Builder", desc: "Own 11+ players", earned: stats.players >= 11, icon: "👥" },
    { name: "Veteran", desc: "Play 10 games", earned: (stats.wins + stats.losses) >= 10, icon: "⭐" },
    { name: "Playoff Champion", desc: "Win 25 games", earned: stats.wins >= 25, icon: "👑" },
    { name: "Super Bowl Legend", desc: "Win 100 games", earned: stats.wins >= 100, icon: "🏈" },
  ];
