...
export function TransferMarket({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("american_football_players").select("*").eq("user_id", user.id).order("overall_rating", { ascending: false }).then(({ data }) => setMyPlayers(data || []));
  }, [user]);
  ...
  const listPlayer = async (player: any) => {
    ...
    await supabase.from("american_football_players").update({ is_for_sale: true, sale_price: price }).eq("id", player.id);
    await supabase.from("american_football_transfers").insert({ seller_id: user!.id, player_id: player.id, price, status: "listed" });
  ...
  const unlistPlayer = async (player: any) => {
    await supabase.from("american_football_players").update({ is_for_sale: false, sale_price: null }).eq("id", player.id);
