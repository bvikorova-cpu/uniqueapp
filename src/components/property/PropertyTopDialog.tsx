...
export function PropertyTopDialog({ open, onOpenChange, propertyId, propertyTitle, featuredUntil, onSuccess }: Props) {
  ...
  const buy = async (days: number, cost: number) => {
    ...
    try {
      ...
      toast.success("Listing is now TOP", {
        description: `${cost} credits used · Top for ${days} days.`,
      });
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Crown className="h-5 w-5 text-amber-500" /> Top this listing
          </DialogTitle>
          <DialogDescription>
            {propertyTitle ? `"${propertyTitle}" ` : ""}gets a TOP badge and is shown at the top of the marketplace.
          </DialogDescription>
        </DialogHeader>
