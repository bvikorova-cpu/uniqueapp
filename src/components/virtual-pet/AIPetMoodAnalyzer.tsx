...
import { ArrowLeft, SmilePlus, Loader2, Sparkles } from "lucide-react";
...
export const AIPetMoodAnalyzer = ({ onBack }: Props) => {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('pet-mood-analyzer', {
        body: {
      ...
      if (error) throw error;
      for (let i = 0; i < 4; i++) await useCredit("custom_generation", "AI Pet Mood Analyzer");
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="text-center space-y-2">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 mb-2">
          <SmilePlus className="w-8 h-8 text-cyan-500" />
        </div>
        <h2 className="text-2xl font-black bg-gradient-to-r from-foreground via-primary to-accent bg-clip-text text-transparent">AI Pet Mood Analyzer</h2>
        <p className="text-muted-foreground text-sm">Deep emotional state analysis with behavioral insights</p>
        <p className="text-xs text-primary font-semibold">4 Credits per analysis</p>
      </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <Card className="border-cyan-500/20 bg-gradient-to-br from-cyan-500/5 to-background">
            <CardContent className="p-6">
              <h3 className="font-bold mb-3 flex items-center gap-2"><SmilePlus className="w-5 h-5 text-cyan-500" />Mood Analysis</h3>
