...
const DRILLS = [
  ...
  { type: "Speed Training", stat: "speed", cost: 200 },
  { type: "Defense Workshop", stat: "defense", cost: 200 },
  { type: "Passing Clinic", stat: "passing", cost: 200 },
  { type: "Rebounding Camp", stat: "rebounding", cost: 250 },
  { type: "Three-Point Practice", stat: "three_point", cost: 300 },
  { type: "Dunk Training", stat: "dunking", cost: 300 },
...
export function TrainingCenter({ onBack }: { onBack: () => void }) {
  ...
  const train = async (drill: typeof DRILLS[0]) => {
    ...
    try {
      ...
      await supabase.from("basketball_players").update({ [drill.stat]: newVal, overall_rating: newOvr }).eq("id", selectedPlayer);
