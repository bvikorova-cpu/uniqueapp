...
const PLAYSTYLES = ["Balanced", "Fast Break", "Inside Game", "Three-Point Focused", "Defensive Wall", "Motion Offense"];
...
export function TeamBuilder({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("basketball_teams").select("*").eq("user_id", user.id).single().then(({ data }) => { setTeam(data); if (data) { setTeamName(data.name); setPlaystyle(data.playstyle); } });
    supabase.from("basketball_players").select("*").eq("user_id", user.id).order("overall_rating", { ascending: false }).then(({ data }) => setPlayers(data || []));
  }, [user]);

  const saveTeam = async () => {
    if (!user || !teamName.trim()) return;
    if (team) {
      await supabase.from("basketball_teams").update({ name: teamName, playstyle }).eq("id", team.id);
    } else {
      const { data } = await supabase.from("basketball_teams").insert({ user_id: user.id, name: teamName, playstyle }).select().single();
      setTeam(data);
    }
  ...
  const toggleStarter = async (player: any) => {
    ...
    if (!player.is_starter && starters.length >= 5) { toast.error("Max 5 starters!"); return; }
    await supabase.from("basketball_players").update({ is_starter: !player.is_starter }).eq("id", player.id);
...
<div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {team && <div className="text-sm text-muted-foreground">W{team.wins} L{team.losses} | Points: {team.league_points}</div>}
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          {players.length === 0 ? <p className="text-sm text-muted-foreground">No players yet. Create or buy some!</p> :
