...
export function LeagueSystem({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    supabase.from("basketball_leagues").select("*").order("created_at", { ascending: false }).then(({ data }) => setLeagues(data || []));
  }, []);
...
<div className="space-y-4">
      ...
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><Trophy className="h-5 w-5 text-primary" />Basketball Leagues</CardTitle></CardHeader>
        <CardContent>
