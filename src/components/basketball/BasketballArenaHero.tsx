...
export function BasketballArenaHero({ stats, onNavigate }: BasketballArenaHeroProps) {
  ...
  const statItems = [
    { icon: Users, label: "Players", value: stats.totalPlayers, color: "text-orange-400" },
    ...
    <div className="relative rounded-2xl overflow-hidden">
      <div className="relative h-[280px] md:h-[380px]">
        <div className="absolute inset-0">
          <Suspense fallback={<div className="w-full h-full bg-gradient-to-br from-orange-950 to-slate-950" />}>
        ...
        <div className="relative z-10 h-full flex flex-col justify-end p-4 md:p-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="inline-block border-2 border-orange-500/60 rounded-xl px-4 py-2 mb-3 backdrop-blur-sm bg-black/30">
              <h1 className="text-2xl md:text-5xl font-black text-white" style={{ textShadow: "0 2px 20px rgba(249,115,22,0.5)" }}>
                🏀 Basketball Arena
              </h1>
            </div>
