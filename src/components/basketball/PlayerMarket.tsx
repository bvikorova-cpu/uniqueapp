...
export function PlayerMarket({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    supabase.from("basketball_players").select("*").eq("is_for_sale", true).order("overall_rating", { ascending: false }).then(({ data }) => setListings((data || []).filter(p => p.user_id !== user?.id)));
  }, [user]);
            ...
            <div className="space-y-3">{listings.map(p => (
              <div key={p.id} className="flex items-center justify-between p-3 rounded-lg border border-border/50">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">SHT:{p.shooting} SPD:{p.speed} DEF:{p.defense} 3PT:{p.three_point}</div>
                </div>
