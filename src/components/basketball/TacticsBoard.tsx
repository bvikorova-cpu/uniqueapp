  ...
  const analyze = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("generate-gift-message", {
        ...
        body: {
          prompt: `As a basketball tactical analyst, analyze:\nTeam: ${team?.name || "My Team"}\nPlaystyle: ${team?.playstyle || "Balanced"}\nPlayers: ${(players || []).map(p => `${p.name}(${p.position},OVR:${p.overall_rating},SHT:${p.shooting},SPD:${p.speed},DEF:${p.defense},3PT:${p.three_point})`).join(", ")}\n\nProvide:\n1. Optimal lineup and rotations\n2. Offensive strategy\n3. Defensive scheme\n4. Key matchup advantages\n5. Areas to improve\n6. Recommended plays`,
          type: "basketball_tactics"
        }
      });
