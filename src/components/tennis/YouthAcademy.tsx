...
export function YouthAcademy({ onBack }: { onBack: () => void }) {
  ...
  const discover = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("generate-gift-message", {
        ...
        body: {
          prompt: `Generate 3 young tennis prospects (age 14-19) as JSON array: [{"name": "<realistic name>", "age": <14-19>, "position": "<singles/doubles/allround>", "overall_rating": <45-65>, "potential": <80-99>, "serve": <30-70>, "forehand": <30-70>, "backhand": <30-65>, "speed": <40-80>, "trait": "<unique trait like 'Big Server' or 'Clay Court Specialist' or 'Net Rusher'>", "development_cost": <200-600>}]`,
          type: "tennis_youth"
        }
      });
      ...
      await supabase.from("tennis_coins").update({ balance: coins.balance - 350, total_spent: coins.total_spent + 350 }).eq("user_id", user.id);
  ...
  const signProspect = async (p: any) => {
    ...
    await supabase.from("tennis_coins").update({ balance: coins.balance - p.development_cost, total_spent: coins.total_spent + p.development_cost }).eq("user_id", user.id);
    await supabase.from("tennis_players").insert({ user_id: user.id, name: p.name, position: p.position, overall_rating: p.overall_rating, serve: p.serve, forehand: p.forehand, backhand: p.backhand, volley: 30 + Math.floor(Math.random() * 30), speed: p.speed, stamina: 40 + Math.floor(Math.random() * 30), market_value: p.development_cost * 2 });
