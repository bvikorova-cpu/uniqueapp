...
const UPGRADES = [
  ...
  { name: "Premium Court Surface", field: "court_type", add: 0, cost: 2000, desc: "Upgrade court quality" },
...
export function StadiumBuilder({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("tennis_stadiums").select("*").eq("user_id", user.id).single().then(async ({ data }) => {
      if (data) { setStadium(data); } else {
        const { data: created } = await supabase.from("tennis_stadiums").insert({ user_id: user.id }).select().single();
        setStadium(created);
      }
    });
  }, [user]);
  ...
  const upgrade = async (upg: typeof UPGRADES[0]) => {
    ...
    await supabase.from("tennis_coins").update({ balance: coins.balance - upg.cost, total_spent: coins.total_spent + upg.cost }).eq("user_id", user.id);
    ...
    await supabase.from("tennis_stadiums").update(updates).eq("id", stadium.id);
...
<div className="space-y-4">
      ...
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><Building className="h-5 w-5 text-primary" />Court Builder</CardTitle></CardHeader>
