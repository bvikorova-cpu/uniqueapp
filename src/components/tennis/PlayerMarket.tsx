...
export function PlayerMarket({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    supabase.from("tennis_players").select("*").eq("is_for_sale", true).order("overall_rating", { ascending: false }).then(({ data }) => setListings((data || []).filter(p => p.user_id !== user?.id)));
  }, [user]);
  ...
  const buyPlayer = async (player: any) => {
    ...
    await supabase.from("tennis_coins").update({ balance: coins.balance - price, total_spent: coins.total_spent + price }).eq("user_id", user.id);
    await supabase.from("tennis_players").update({ user_id: user.id, is_for_sale: false, sale_price: null }).eq("id", player.id);
    await supabase.from("tennis_transfers").insert({ seller_id: player.user_id, buyer_id: user.id, player_id: player.id, price, status: "completed" });
            ...
            <div className="space-y-3">{listings.map(p => (
              <div key={p.id} className="flex items-center justify-between p-3 rounded-lg border border-border/50">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">SRV:{p.serve} FH:{p.forehand} BH:{p.backhand} SPD:{p.speed}</div>
                </div>
