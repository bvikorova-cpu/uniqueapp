...
const POSITIONS = [
  ...
  { value: "allround", label: "All-Rounder" },
];

export function PlayerCreator({ onBack }: { onBack: () => void }) {
  const { user } = useAuth();
  const [name, setName] = useState("");
  const [position, setPosition] = useState("singles");
  ...
  const createPlayer = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("generate-gift-message", {
        ...
        body: {
          prompt: `Generate a tennis player as JSON: {"name": "${name}", "position": "${position}", "overall_rating": <55-90>, "serve": <40-95>, "forehand": <40-95>, "backhand": <40-95>, "volley": <40-95>, "speed": <40-95>, "stamina": <40-95>, "market_value": <1000-15000>, "description": "<one sentence about playstyle>"}`,
          type: "tennis_player_create"
        }
      });
      ...
      await supabase.from("tennis_coins").update({ balance: coins.balance - 500, total_spent: coins.total_spent + 500 }).eq("user_id", user.id);
      const { data: inserted } = await supabase.from("tennis_players").insert({ user_id: user.id, name: player.name, position: player.position, overall_rating: player.overall_rating, serve: player.serve, forehand: player.forehand, backhand: player.backhand, volley: player.volley, speed: player.speed, stamina: player.stamina, market_value: player.market_value }).select().single();
      setCreated(inserted);
          ...
          {created && (
            <div className="p-4 rounded-lg bg-primary/10 border border-primary/20 space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2 text-xs">
                <div>🎾 Serve: {created.serve}</div><div>💪 Forehand: {created.forehand}</div><div>🔄 Backhand: {created.backhand}</div>
                <div>🏐 Volley: {created.volley}</div><div>🏃 Speed: {created.speed}</div><div>❤️ Stamina: {created.stamina}</div>
                <div>💰 Value: {created.market_value}</div>
              </div>
            </div>
          )}
        </CardContent>
