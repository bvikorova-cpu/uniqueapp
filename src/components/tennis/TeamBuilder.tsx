...
const PLAYSTYLES = ["Baseline", "Serve & Volley", "Aggressive Baseliner", "Counter-Puncher", "All-Court", "Net Rusher"];
...
export function TeamBuilder({ onBack }: { onBack: () => void }) {
  ...
  const [playstyle, setPlaystyle] = useState("Baseline");

  useEffect(() => {
    if (!user) return;
    supabase.from("tennis_teams").select("*").eq("user_id", user.id).single().then(({ data }) => { setTeam(data); if (data) { setTeamName(data.name); setPlaystyle(data.playstyle); } });
    supabase.from("tennis_players").select("*").eq("user_id", user.id).order("overall_rating", { ascending: false }).then(({ data }) => setPlayers(data || []));
  }, [user]);

  const saveTeam = async () => {
    if (!user || !teamName.trim()) return;
    if (team) {
      await supabase.from("tennis_teams").update({ name: teamName, playstyle }).eq("id", team.id);
    } else {
      const { data } = await supabase.from("tennis_teams").insert({ user_id: user.id, name: teamName, playstyle }).select().single();
      setTeam(data);
    }
  ...
  const toggleStarter = async (player: any) => {
    ...
    await supabase.from("tennis_players").update({ is_starter: !player.is_starter }).eq("id", player.id);
