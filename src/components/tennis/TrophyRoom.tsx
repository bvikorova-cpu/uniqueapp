...
export function TrophyRoom({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    if (!user) return;
    Promise.all([
      supabase.from("tennis_teams").select("*").eq("user_id", user.id).single(),
      supabase.from("tennis_players").select("id", { count: "exact", head: true }).eq("user_id", user.id),
  ...
  const trophies = [
    ...
    { name: "Grand Slam", desc: "Win 25 matches", earned: stats.wins >= 25, icon: "👑" },
