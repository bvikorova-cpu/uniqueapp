...
export function TacticsBoard({ onBack }: { onBack: () => void }) {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const { data: players } = await supabase.from("tennis_players").select("*").eq("user_id", user.id);
      const { data, error } = await supabase.functions.invoke("generate-gift-message", {
        headers: { Authorization: `Bearer ${session.access_token}` },
        body: {
          prompt: `As a tennis tactical analyst, analyze:\nTeam: ${team?.name || "My Team"}\nPlaystyle: ${team?.playstyle || "Baseline"}\nPlayers: ${(players || []).map(p => `${p.name}(${p.position},OVR:${p.overall_rating},SRV:${p.serve},FH:${p.forehand},BH:${p.backhand},SPD:${p.speed})`).join(", ")}\n\nProvide:\n1. Optimal singles lineup\n2. Doubles pairing recommendations\n3. Surface-specific strategy (clay, grass, hard)\n4. Return game tactics\n5. Serve patterns analysis\n6. Areas to improve`,
          type: "tennis_tactics"
        }
      });
      if (error) throw error;
      await supabase.from("tennis_coins").update({ balance: coins.balance - 300, total_spent: coins.total_spent + 300 }).eq("user_id", user.id);
