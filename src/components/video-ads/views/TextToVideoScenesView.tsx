...
export const TextToVideoScenesView = ({ onBack }: { onBack: () => void }) => {
  ...
  const split = async () => {
    ...
    try {
      ...
      toast.success(`${data.result.totalScenes} scén pripravených (${data.credits_used} CR)`);
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div><h2 className="text-2xl font-black">Text-to-Video Scenes</h2><p className="text-sm text-muted-foreground">Runway/Sora-štýl B-roll z textu</p></div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div><Label>Script *</Label><Textarea rows={6} value={script} onChange={e => setScript(e.target.value)} placeholder="Vlož celý voiceover/script..." /></div>
            ...
            <div><Label>Štýl</Label><input className="w-full mt-1 p-2 rounded-md border bg-background" value={style} onChange={e => setStyle(e.target.value)} /></div>
            <Button onClick={split} disabled={loading} className="w-full bg-gradient-to-r from-violet-500 to-fuchsia-600">
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Wand2 className="mr-2 h-4 w-4" />1) Rozdeliť na scény (3 CR)</>}
            </Button>
            {splitResult && (
              <Button onClick={renderVideo} disabled={genVideo} variant="outline" className="w-full">
                {genVideo ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Play className="mr-2 h-4 w-4" />2) Vygenerovať frames (5 CR)</>}
              </Button>
            )}
          </CardContent>
        </Card>
        <Card className="lg:col-span-2">
          <CardHeader><CardTitle>Scény</CardTitle></CardHeader>
          <CardContent className="max-h-[700px] overflow-y-auto space-y-3">
            {!splitResult ? <p className="text-muted-foreground text-center py-12">Vlož script a rozdeľ na scény</p> : (
              <>
                {frames.length > 0 && (
                  <div className={`relative ${aspect === '9:16' ? 'aspect-[9/16] max-w-xs mx-auto' : aspect === '1:1' ? 'aspect-square' : 'aspect-video'} bg-black rounded-lg overflow-hidden mb-4`}>
                    ...
                    <Badge className="absolute top-2 right-2 bg-black/60">Scéna {activeFrame + 1}/{frames.length}</Badge>
                  </div>
                )}
                <div className="grid grid-cols-2 gap-2 text-xs"><Badge variant="outline">📐 {splitResult.suggestedAspect}</Badge><Badge variant="outline">⏱ {splitResult.totalDuration}s</Badge><Badge variant="outline">🎬 {splitResult.totalScenes} scén</Badge></div>
                ...
                {splitResult.scenes.map(s => (
                  <Card key={s.sceneNumber} className="bg-muted/30"><CardContent className="pt-3 text-sm space-y-1">
                    <div className="font-bold flex justify-between">Scéna {s.sceneNumber} <Badge variant="secondary">{s.durationSeconds}s</Badge></div>
                    <p>{s.visualPrompt}</p>
