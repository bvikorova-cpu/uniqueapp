...
import { Film, Loader2, Download, Sparkles, Play, ImagePlus, X } from "lucide-react";
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1">
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <Label>Your own photos (optional)</Label>
              <p className="text-xs text-muted-foreground mb-2">
                Up to 5 images — they are used as scenes in order, the rest is generated by AI.
              </p>
              <input
                ref={fileRef}
                type="file"
              ...
              />
              <Button type="button" variant="outline" className="w-full" onClick={() => fileRef.current?.click()} disabled={busy}>
                <ImagePlus className="w-4 h-4 mr-2" />Add photos ({photos.length}/5)
              </Button>
              {photos.length > 0 && (
