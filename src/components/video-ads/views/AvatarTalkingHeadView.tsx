...
import { Loader2, UserCircle2, Image as ImageIcon } from "lucide-react";
...
export const AvatarTalkingHeadView = ({ onBack }: { onBack: () => void }) => {
  ...
  const generatePlan = async () => {
    ...
    setLoading(true); setPlan(null); setPortrait(null);
    try {
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <Badge className="ml-auto bg-gradient-to-r from-sky-500 to-indigo-600 text-white">3+5 CR</Badge>
      </div>
          ...
          <CardContent className="max-h-[700px] overflow-y-auto space-y-4">
            {!plan ? <p className="text-muted-foreground text-center py-12">Generate plan</p> : (
              <>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs"><Badge variant="outline">⚡ {plan.pacing}</Badge><Badge variant="outline">🏛 {plan.backgroundType}</Badge><Badge variant="outline">👔 {plan.dressCode}</Badge></div>
