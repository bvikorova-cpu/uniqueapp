...
export const VideoResizerView = ({ onBack }: { onBack: () => void }) => {
  ...
  const go = async () => {
    ...
    try {
      ...
      setR(data.result);
      toast.success(`${data.result.targets.length} formátov pripravených (${data.credits_used} CR)`);
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div><h2 className="text-2xl font-black">Video Resizer / Auto-Crop</h2><p className="text-sm text-muted-foreground">Reframing pre všetky platformy</p></div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div><Label>Zdrojový pomer</Label>
            ...
            <div><Label>Hlavný subjekt</Label><input className="w-full mt-1 p-2 rounded-md border bg-background" value={subject} onChange={e => setSubject(e.target.value)} placeholder="center, left third, person face..." /></div>
            <div><Label>On-screen text</Label>
              <select className="w-full mt-1 p-2 rounded-md border bg-background" value={hasText} onChange={e => setHasText(e.target.value)}>
                <option value="yes">Áno</option><option value="no">Nie</option>
              </select>
            </div>
            <div><Label>Cieľové formáty</Label><textarea className="w-full mt-1 p-2 rounded-md border bg-background" rows={2} value={targets} onChange={e => setTargets(e.target.value)} /></div>
            <Button onClick={go} disabled={loading} className="w-full bg-gradient-to-r from-orange-500 to-pink-600">
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Generovať plán (3 CR)'}
            </Button>
          </CardContent>
        </Card>
        <Card className="lg:col-span-2">
          <CardHeader><CardTitle>Reframing plán</CardTitle></CardHeader>
          <CardContent className="max-h-[700px] overflow-y-auto space-y-3">
            {!r ? <p className="text-muted-foreground text-center py-12">Vyplň a vygeneruj</p> : (
              <>
                ...
                <div><h4 className="font-semibold mb-1">⚠️ Časté chyby</h4><ul className="text-sm">{r.commonMistakes.map((m, i) => <li key={i}>• {m}</li>)}</ul></div>
                <div><h4 className="font-semibold mb-1 flex justify-between items-center">FFmpeg príkaz <Button size="sm" variant="ghost" onClick={() => { navigator.clipboard.writeText(r.ffmpegCommandHint); toast.success('Skopírované'); }}><Copy className="w-3 h-3" /></Button></h4>
