...
export function HealthyComments({ submissionId }: { submissionId: string }) {
  const { user } = useAuth();
  const [open, setOpen] = useState(false);
  const [comments, setComments] = useState<HComment[]>([]);
          ...
          {user ? (
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" onClick={post} disabled={posting || body.trim().length === 0} className="bg-orange-600 hover:bg-orange-700">
