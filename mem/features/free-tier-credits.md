...
description: 10 free credits at signup + 10 each 1st of month (Europe/City). Paid plans on top.
type: feature
---
# Free Tier Credits

- **Signup grant**: 10 credits via `handle_new_user_free_credits` trigger.
- **Monthly top-up**: +10 credits on 1st of each month (Europe/City TZ) via `ensure_free_tier_credits` RPC. Idempotent per `month_key` (YYYY-MM).
