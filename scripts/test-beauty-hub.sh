#!/usr/bin/env bash
# Megakontrola pre /beauty-hub (BeautyStudio)
# - Routes
# - RLS (anon dotazy musia vrátiť 0 / unauthorized)
...
echo "=== 1. Routes ==="
for path in /beauty-hub /beauty-studio /ai-credits-store /auth; do
  st=$(curl -s -o /dev/null -w "%{http_code}" "${APP_URL}${path}")
  check_status "GET $path" "200 301 302 304 404" "$st"
done

echo ""
echo "=== 2. Anonymous RLS lockdown (must return [] or JWT error) ==="
for tbl in beauty_transformations beauty_celebrity_matches beauty_skin_analyses \
           beauty_nail_designs beauty_tutorials beauty_product_recommendations beauty_gallery; do
  body=$(curl -s -H "apikey: $ANON_KEY" "${SUPABASE_URL}/rest/v1/${tbl}?select=*&limit=1")
  if [ "$body" = "[]" ] || echo "$body" | grep -qE "JWT|permission"; then
...
FNS=(beauty-celebrity-match beauty-nail-art beauty-recommendations \
     beauty-skin-analysis beauty-transformation beauty-tutorial)
for fn in "${FNS[@]}"; do
...
for fn in beauty-tutorial beauty-skin-analysis beauty-transformation; do
...
echo ""
echo "=== 5. Stripe create-checkout reachable (expect 401 without JWT) ==="
st=$(curl -s -o /dev/null -w "%{http_code}" -X POST \
  -H "apikey: $ANON_KEY" -H "Content-Type: application/json" \
...
if [ -n "$ACCESS_TOKEN" ]; then
  ...
  st=$(curl -s -o /dev/null -w "%{http_code}" -X POST \
    ...
    -H "Content-Type: application/json" -d '{"lookDescription":"test"}' \
    "${SUPABASE_URL}/functions/v1/beauty-tutorial")
  check_status "POST beauty-tutorial (auth)" "200 402" "$st"
fi
